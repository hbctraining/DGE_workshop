...
3. Download the [Salmon files using this link](https://www.dropbox.com/s/shuaxoiecxx78mh/salmon.zip?dl=0) and move the zip archive (`salmon.zip`) to the `data` directory. _These are the full dataset Salmon files that contain transcript abundances for each sample._
4. Decompress (unzip) the zip archive by double-clicking on the file. You should now see a `salmon` folder in your `data` directory.
...
> **NOTE:** You should have these packages installed, but if not the code to do so is provided below:
> 
> ```R
