...
We first need to load the libraries and subset our results to only include those significant results based on p-adjusted value. Depending on the experiment, you may also want to include a log2 fold-change threshold.
...
To use clusterProfiler, we are going to turn our gene names into Ensembl gene IDs using a few clusterProfiler functions. We will also acquire the corresponding Entrez gene IDs for additional downstream analyses.
...
Now we would like to subset the results to only return significant genes (padj < 0.05):
...
## [Other functional analysis methods](https://github.com/hbctraining/In-depth-NGS-Data-Analysis-Course/blob/may2017/sessionIII/lessons/functional_analysis_other_methods.md)
